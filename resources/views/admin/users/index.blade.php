...
@section('content')
    <div class="admin-list-page">
        <header class="admin-list-hero">
            ...
            <h2 class="admin-list-title">Users</h2>
            <p class="admin-list-lede">
                Manage community accounts and admins — ban, unban, or remove empty profiles.
            </p>
            <div class="admin-list-meta">
                <span class="admin-list-chip">{{ number_format($userCount) }} users</span>
                <span class="admin-list-chip">{{ number_format($adminCount) }} {{ \Illuminate\Support\Str::plural('admin', $adminCount) }}</span>
        ...
        <nav class="admin-list-tabs" aria-label="User types">
            <a
                href="{{ route('admin.users.index', array_filter(['tab' => 'users', 'search' => request('search'), 'sort' => request('sort')])) }}"
                class="admin-list-tab {{ $tab === 'users' ? 'active' : '' }}"
            >
                Users
                <span class="admin-list-tab-count">{{ number_format($userCount) }}</span>
            </a>
            <a
                href="{{ route('admin.users.index', array_filter(['tab' => 'admins', 'search' => request('search'), 'sort' => request('sort')])) }}"
                class="admin-list-tab {{ $tab === 'admins' ? 'active' : '' }}"
            >
                Admins
                <span class="admin-list-tab-count">{{ number_format($adminCount) }}</span>
            </a>
        </nav>

        <form method="GET" action="{{ route('admin.users.index') }}" class="admin-list-toolbar filter-bar">
        ...
        @else
            ...
            <div class="admin-users-table admin-list-table">
